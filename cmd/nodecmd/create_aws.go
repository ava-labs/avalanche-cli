...
func getNewKeyPairName(ec2Svc *awsAPI.AwsCloud) (string, error) {
	...
	for {
		if newKeyPairName != "" {
			...
			ux.Logger.PrintToUser(fmt.Sprintf("Key Pair named %s already exists", newKeyPairName)) //nolint:govet
		}
	...
	for _, region := range regions {
		...
		if replaceKeyPair && !forMonitoring {
			...
			if !keyPairExists {
				...
				case !useSSHAgent && !certInSSHDir:
					ux.Logger.PrintToUser(fmt.Sprintf("Creating new key pair %s in AWS[%s]", keyPairName, region)) //nolint:govet
					if err := ec2Svc[region].CreateAndDownloadKeyPair(regionConf[region].Prefix, privKey); err != nil {
						return instanceIDs, elasticIPs, sshCertPath, keyPairName, err
					}
				}
		...
		if !securityGroupExists {
			ux.Logger.PrintToUser(fmt.Sprintf("Creating new security group %s in AWS[%s]", securityGroupName, region)) //nolint:govet
			...
			sgID = *sg.GroupId
			ux.Logger.PrintToUser(fmt.Sprintf("Using existing security group %s in AWS[%s]", securityGroupName, region)) //nolint:govet
	...
	if err != nil {
		...
		for region, regionInstanceID := range instanceIDs {
			for _, instanceID := range regionInstanceID {
				ux.Logger.PrintToUser(fmt.Sprintf("Destroying AWS cloud server %s...", instanceID)) //nolint:govet
				if destroyErr := ec2Svc[region].DestroyInstance(instanceID, "", true); destroyErr != nil {
					failedNodes[instanceID] = destroyErr
				}
				ux.Logger.PrintToUser(fmt.Sprintf("AWS cloud server instance %s destroyed", instanceID)) //nolint:govet
			}
		}
