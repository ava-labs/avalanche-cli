...
func destroyNodes(_ *cobra.Command, args []string) error {
	...
	for _, node := range nodesToStop {
		...
		if nodeConfig.CloudService == "" || nodeConfig.CloudService == constants.AWSCloudService {
			...
			if err = deleteMonitoringSecurityGroupRule(map[string]*awsAPI.AwsCloud{nodeConfig.Region: ec2Svc}, nodeConfig.ElasticIP, nodeConfig.SecurityGroup, nodeConfig.Region); err != nil {
				return err
			}
		} else {
