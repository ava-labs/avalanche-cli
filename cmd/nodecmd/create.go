...
func createNodes(_ *cobra.Command, args []string) error {
	...
	if cloudService == constants.AWSCloudService { // Get AWS Credential, region and AMI
		...
		if separateMonitoringInstance {
			...
			monitoringEc2SvcMap[regions[0]] = ec2SvcMap[regions[0]]
			monitoringNumNodesMap := make(map[string]int)
			monitoringNumNodesMap[regions[0]] = 1
			monitoringCloudConfig, err := createAWSInstances(monitoringEc2SvcMap, nodeType, monitoringNumNodesMap, []string{regions[0]}, ami, usr, true)
			if err != nil {
				return err
			}
		...
		if !useStaticIP {
			monitoringPublicIPMap, err := awsAPI.GetInstancePublicIPs(ec2Svc, monitoringNodeConfig.InstanceIDs)
			if err != nil {
				return err
			}
