...
func createNodes(_ *cobra.Command, args []string) error {
	...
	if cloudService == constants.AWSCloudService { // Get AWS Credential, region and AMI
		ec2SvcMap, ami, err := getAWSCloudConfig(awsProfile)
		...
		cloudConfigMap, err = createGCPInstance(usr, gcpClient, nodeType, numNodes, zones, imageID, credentialFilepath, clusterName)
		if err != nil {
			return err
		}
