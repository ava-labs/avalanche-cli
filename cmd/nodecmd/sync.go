...
func trackSubnet(clusterName, subnetName string, network models.Network) ([]string, error) {
	...
	if err := ansible.RunAnsiblePlaybookSetupCLIFromSource(app.GetAnsibleDir(), app.GetAnsibleInventoryDirPath(clusterName), "create-devnets-explore-endpoints", "all"); err != nil {
		return nil, err
	}
