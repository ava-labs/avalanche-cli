...
func checkAvalancheGoVersionCompatible(clusterName, subnetName string) error {
	...
	err := app.CreateFile(app.GetAvalancheGoJSONFile())
	if err != nil {
		return err
	}
...
func syncSubnet(_ *cobra.Command, args []string) error {
	...
	if _, err := subnet.ValidateSubnetNameAndGetChains([]string{subnetName}); err != nil {
		return err
	}
...
func trackSubnet(clusterName, subnetToTrack string, network models.Network) error {
	...
	if err := subnet.CallExportSubnet(subnetToTrack, subnetPath, network); err != nil {
		return err
	}
