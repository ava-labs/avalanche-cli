...
func resize(_ *cobra.Command, args []string) error {
	...
	for _, node := range nodesToResize {
		...
		if nodeType != "" {
			spinner := spinSession.SpinToUser(utils.ScriptLog(nodeConfig.NodeID, "Resizing Instance Type")) //nolint:govet
		...
		if diskSize != "" {
			spinner := spinSession.SpinToUser(utils.ScriptLog(nodeConfig.NodeID, "Resizing Disk")) //nolint:govet
