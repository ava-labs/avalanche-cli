...
var ErrNoSubnetAuthKeysInWallet = errors.New("auth wallet does not contain blockchain auth keys")
...
func (d *PublicDeployer) AddValidatorNonSOV(
	...
	if err != nil {
		return false, nil, nil, fmt.Errorf("failure parsing blockchain auth keys: %w", err)
	}
...
func (d *PublicDeployer) DeployBlockchain(
	...
	subnetAuthKeys, err := address.ParseToIDs(subnetAuthKeysStrs)
	if err != nil {
		return false, ids.Empty, nil, nil, fmt.Errorf("failure parsing blockchain auth keys: %w", err)
	}
...
func (d *PublicDeployer) Sign(
	...
	subnetAuthKeys, err := address.ParseToIDs(subnetAuthKeysStrs)
	if err != nil {
		return fmt.Errorf("failure parsing blockchain auth keys: %w", err)
	}
