...
func getExpectedDefaultSnapshotSHA256Sum(oneNode bool) (string, error) {
	url := constants.BootstrapSnapshotSHA256URL
	if oneNode {
		url = constants.BootstrapSnapshot1NodeSHA256URL
	}
	...
	if oneNode {
		path = constants.BootstrapSnapshot1NodeLocalPath
	}
...
func SetDefaultSnapshot(snapshotsDir string, force bool, oneNode bool) error {
	bootstrapSnapshotArchivePath := filepath.Join(snapshotsDir, constants.BootstrapSnapshotArchiveName)
	if oneNode {
		bootstrapSnapshotArchivePath = filepath.Join(snapshotsDir, constants.BootstrapSnapshot1NodeArchiveName)
	}
	...
	if _, err := os.Stat(bootstrapSnapshotArchivePath); os.IsNotExist(err) {
		...
		expectedSum, err := getExpectedDefaultSnapshotSHA256Sum(oneNode)
		if err != nil {
