...
func StartLocalNode(
	...
	if localnet.LocalClusterExists(app, clusterName) {
		...
		case network.Kind == models.Mainnet:
			ux.Logger.PrintToUser("%s", logging.Yellow.Wrap("Warning: Mainnet Bootstrapping can take 6-24 hours"))
			connectionSettings.NetworkID = network.ID
...
func LocalStatus(
	...
	if clusterName != "" {
		...
		ux.Logger.PrintToUser("%s", logging.Blue.Wrap("Local clusters:"))
	}
	for _, clusterName := range localClusters {
		...
		ux.Logger.PrintToUser("%s", avagoURIOuput)
	}
