...
func createKeyPair() {
	...
	fmt.Println("DEBUG: mkdir")
	gomega.Expect(err).Should(gomega.BeNil())
	privateKey, err := rsa.GenerateKey(rand.Reader, 2048)
	fmt.Println("DEBUG: generate key")
	gomega.Expect(err).Should(gomega.BeNil())
	privateKeyFile, err := os.Create(privateKeyPath)
	fmt.Println("DEBUG: create key file")
	gomega.Expect(err).Should(gomega.BeNil())
	defer privateKeyFile.Close()
	privateKeyPEM := &pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(privateKey)}
	err = pem.Encode(privateKeyFile, privateKeyPEM)
	fmt.Println("DEBUG: pem encode")
	gomega.Expect(err).Should(gomega.BeNil())
	pub, err := ssh.NewPublicKey(&privateKey.PublicKey)
	fmt.Println("DEBUG: new public key")
	gomega.Expect(err).Should(gomega.BeNil())
	err = os.WriteFile(pubKeyPath, ssh.MarshalAuthorizedKey(pub), 0o600)
	fmt.Println("DEBUG: write file")
	gomega.Expect(err).Should(gomega.BeNil())
}
