...
var _ = ginkgo.Describe("[Blockchain Deploy Flags]", ginkgo.Ordered, func() {
	...
	ginkgo.It("HAPPY PATH: local deploy convert only", func() {
		...
		//gomega.Expect(err).Should(gomega.BeNil())
		//subnetID, _ := ids.FromString("WdxR2ErnM56d7gUkm5WVWH2R73R6TmAUQUBDmCRK8Hbb9XUHx")
		subnetID, _ := ids.FromString("2W9boARgCWL25z6pMFNtkCfNA5v28VGg9PmBgUJfuKndEdhrvw")
		subnetInfo, _ := blockchain.GetSubnet(subnetID, models.NewLocalNetwork())
		fmt.Printf("obtained subnetInfo %s \n", subnetInfo)
		fmt.Printf("obtained subnetInfo manager addr %s \n", string(subnetInfo.ManagerAddress))
		validatorManagerAddress := "0x" + hex.EncodeToString(subnetInfo.ManagerAddress)
		fmt.Printf("validatorManagerAddress %s \n", validatorManagerAddress)
		//valType, _ := validatorManagerSDK.GetValidatorManagerType("http://127.0.0.1:64249/ext/bc/k8V2Y5Gt3ivFb3PWgo9mU4BqtYadKCq7642BF4zihhSRxV2Um/rpc", common.HexToAddress(validatorManagerAddress))
		valType, _ := validatorManagerSDK.GetValidatorManagerType("http://127.0.0.1:64147/ext/bc/2YuHpbVKMGaJjf2KobCJcNJWyCA93vzxw9xWStjJAQq6QMwjXU/rpc", common.HexToAddress(validatorManagerAddress))

		fmt.Printf("obtaine dtype %s \n", valType)
		//fmt.Printf("obtaine error %s \n", err.Error())
	})
