...
func checkConvertOnlyOutput(output string, generateNodeID bool, subnetName string) {
	...
	gomega.Expect(output).ShouldNot(gomega.ContainSubstring("L1 is successfully deployed on Granite Network"))
	if generateNodeID {
...
var _ = ginkgo.Describe("[Blockchain Deploy]", ginkgo.Ordered, func() {
	...
	ginkgo.AfterEach(func() {
		...
		listSigAggCmd := exec.Command("./bin/avalanche", "interchain", "signatureAggregator", "list", "--granite")
		outputBytes, err := listSigAggCmd.CombinedOutput()
		gomega.Expect(err).Should(gomega.BeNil())
		output := string(outputBytes)
		gomega.Expect(output).Should(gomega.ContainSubstring("No locally run signature aggregator found for Granite Network"))
	...
	ginkgo.It("HAPPY PATH: Granite deploy default", func() {
		testFlags := utils.TestFlags{}
		output, err := utils.TestCommand(cmd.BlockchainCmd, "deploy", blockchainCmdArgs, globalFlags, testFlags)
		gomega.Expect(output).Should(gomega.ContainSubstring("L1 is successfully deployed on Granite Network"))
		gomega.Expect(err).Should(gomega.BeNil())
		...
		gomega.Expect(err).Should(gomega.BeNil())
		subnetInfo, _ := blockchain.GetSubnet(sc.Networks["Granite"].SubnetID, models.NewGraniteNetwork())
		validatorManagerAddress := "0x" + hex.EncodeToString(subnetInfo.ManagerAddress)
		uri := fmt.Sprintf("%s/ext/bc/%s/rpc", localClusterUris[0], sc.Networks["Granite"].BlockchainID)
		...
		gomega.Expect(err).Should(gomega.BeNil())
		output = string(outputBytes)
		gomega.Expect(output).ShouldNot(gomega.ContainSubstring("No locally run signature aggregator found for Granite Network"))
	})
	...
	ginkgo.It("HAPPY PATH: granite deploy convert only", func() {
		testFlags := utils.TestFlags{
			"convert-only": true,
		}
		output, err := utils.TestCommand(cmd.BlockchainCmd, "deploy", blockchainCmdArgs, globalFlags, testFlags)
	...
	ginkgo.It("HAPPY PATH: generate node id ends in convert only", func() {
		...
		sc, err := utils.GetSideCar(blockchainCmdArgs[0])
		gomega.Expect(err).Should(gomega.BeNil())
		numValidators := len(sc.Networks["Granite"].BootstrapValidators)
		gomega.Expect(numValidators).Should(gomega.BeEquivalentTo(1))
		gomega.Expect(sc.Networks["Granite"].BootstrapValidators[0].NodeID).ShouldNot(gomega.BeNil())
		gomega.Expect(sc.Networks["Granite"].BootstrapValidators[0].BLSProofOfPossession).ShouldNot(gomega.BeNil())
		gomega.Expect(sc.Networks["Granite"].BootstrapValidators[0].BLSPublicKey).ShouldNot(gomega.BeNil())
	})
	...
	ginkgo.It("HAPPY PATH: local deploy with bootstrap validator balance", func() {
		...
		output, err := utils.TestCommand(cmd.BlockchainCmd, "deploy", blockchainCmdArgs, globalFlags, testFlags)
		gomega.Expect(output).Should(gomega.ContainSubstring("L1 is successfully deployed on Granite"))
		gomega.Expect(err).Should(gomega.BeNil())
		...
		testFlags = utils.TestFlags{
			"granite":       true,
			"validation-id": sc.Networks["Granite"].BootstrapValidators[0].ValidationID,
		}
