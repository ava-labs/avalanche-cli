...
var _ = ginkgo.Describe("[Local Subnet SOV]", ginkgo.Ordered, func() {
	...
	ginkgo.It("can deploy multiple subnets to local SOV", func() {
		commands.CreateSubnetEvmConfigSOVWithGenesisPath(subnetName, utils.SubnetEvmGenesisPoaPath, commands.PoaValidatorManagerOwner)
		commands.CreateSubnetEvmConfigSOVWithGenesisPath(secondSubnetName, utils.SubnetEvmGenesis2Path, commands.PoaValidatorManagerOwner)
	...
	ginkgo.It("can deploy custom chain config SOV", func() {
		commands.CreateSubnetEvmConfigSOVWithGenesisPath(subnetName, utils.SubnetEvmAllowFeeRecpPath, commands.PoaValidatorManagerOwner)
...
var _ = ginkgo.Describe("[Subnet Compatibility]", func() {
	...
	ginkgo.It("can deploy a subnet-evm with old version SOV", func() {
		...
		commands.CreateSubnetEvmConfigGenesisPathWithVersionSOV(subnetName, utils.SubnetEvmGenesisPoaPath, subnetEVMVersion, commands.PoaValidatorManagerOwner)
		deployOutput := commands.DeploySubnetLocallySOV(subnetName)
		rpcs, err := utils.ParseRPCsFromOutput(deployOutput)
		if err != nil {
			fmt.Println(deployOutput)
	...
	ginkgo.It("can't deploy conflicting vm versions SOV", func() {
		...
		commands.CreateSubnetEvmConfigGenesisPathWithVersionSOV(subnetName, utils.SubnetEvmGenesisPoaPath, subnetEVMVersion1, commands.PoaValidatorManagerOwner)
		commands.CreateSubnetEvmConfigGenesisPathWithVersionSOV(secondSubnetName, utils.SubnetEvmGenesis2Path, subnetEVMVersion2, commands.PoaValidatorManagerOwner)
