...
var _ = ginkgo.Describe("[Node monitoring]", func() {
	...
	ginkgo.It("verifies correct promtail config", func() {
		...
		gomega.Expect(sshOutput).To(gomega.ContainSubstring(fmt.Sprintf("url: http://%s:%s/loki/api/v1/push", monitoringHost[0].IP, constants.AvalanchegoLokiPort)))
	...
	ginkgo.It("verifies correct loki config", func() {
		...
		gomega.Expect(sshOutput).To(gomega.ContainSubstring(fmt.Sprintf("http_listen_port: %s", constants.AvalanchegoLokiPort)))
