...
// DownloadWithCache provides a mock function with given fields: url, path, duration
func (_m *Downloader) DownloadWithCache(url string, path string, duration time.Duration) ([]byte, error) {
	ret := _m.Called(url, path, duration)

	if len(ret) == 0 {
		panic("no return value specified for DownloadWithCache")
	}

	var r0 []byte
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string, time.Duration) ([]byte, error)); ok {
		return rf(url, path, duration)
	}
	if rf, ok := ret.Get(0).(func(string, string, time.Duration) []byte); ok {
		r0 = rf(url, path, duration)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]byte)
		}
...
// DownloadWithTee provides a mock function with given fields: url, path
func (_m *Downloader) DownloadWithTee(url string, path string) ([]byte, error) {
	ret := _m.Called(url, path)

	if len(ret) == 0 {
		panic("no return value specified for DownloadWithTee")
	}
